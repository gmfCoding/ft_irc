...
void Command::handlePassCommand(IRCClient* client, const std::vector<std::string>& parameters)
{
    ...
    if (parameters.empty()) {
        ...
        client->GetServer()->clientSendData(client->GetFd(), "ERROR :No password provided");
        return;
    }
    ...
    if (password != (std::string)client->GetServer()->GetPassword())
    {
        ...
        client->GetServer()->clientSendData(client->GetFd(), "ERROR :Invalid password");
        return;
    }
    ...
    client->GetServer()->clientSendData(client->GetFd(), "PASS :Password accepted");
